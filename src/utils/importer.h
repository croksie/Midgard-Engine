...
inline std::vector<scene::Model> importModel(const std::string& pFile, bifrost::Bifrost* bifrost, bool useInstance = false) {
    // Import file
    ...
    // Add all meshs
    for(unsigned int modelIndex = 0; modelIndex < scene->mNumMeshes; modelIndex++){

        auto ai_mesh = scene->mMeshes[modelIndex];
        ...
        models.emplace_back(mesh, material);
    }
